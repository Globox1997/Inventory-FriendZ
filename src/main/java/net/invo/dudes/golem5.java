...
public class golem5 extends Item {
    public golem5(Settings settings) {
        super(settings);
    }

    @Override
    public void appendTooltip(ItemStack itemStack, World world, List<Text> tooltip, TooltipContext tooltipContext) {
        tooltip.add(new TranslatableText("item.invo.golem5.tooltip"));
    }

    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        if (slot == 0 || slot == 1 || slot == 2 || slot == 3 || slot == 4 || slot == 5 || slot == 6 || slot == 7
                || slot == 8) {
            StatusEffectInstance haste = new StatusEffectInstance(StatusEffect.byRawId(3), 0, 0, false, false);
            LivingEntity bob = (LivingEntity) entity;
            bob.addStatusEffect(haste);
        }
